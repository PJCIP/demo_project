...
review = st.text_input(label = 'Type your review')
_,score,subject,_,key_entity = sentiment(review)
# print(score)
# print(subject)
...
if len(review) > 0:
    if score > 0:
        # check = 1
        st.image('./templates/up.png')
        st.text('The text is positive sentence')
        if key_entity:
            st.text('The key entitites are :{}'.format(key_entity))
        
    else:
        check = 1
        st.image('./templates/down.png')
        st.text('The text is negative sentence')
        if key_entity:
            st.text('The key entitites are :{}'.format(key_entity))
